...
local MESSAGE_ROW_LEFT_PADDING = 22
...
local FOOTER_LABEL_LEFT_RATIO = 0.31
local FOOTER_PERCENT_RIGHT_RATIO = 0.78
local FOOTER_TRACK_LEFT_RATIO = 0.27
local FOOTER_TRACK_RIGHT_RATIO = 0.80
local FOOTER_TITLE_SIZE = 26
...
local function conversationGeometry(rect)
    local viewport = {
        ...
        w = rect.w * CONVERSATION_RIGHT_RATIO - CONVERSATION_LEFT_INSET,
...
local function drawAngerFooter(painter, rect, model, controller, pixelScale)
    ...
    local labelX = rect.x + rect.w * FOOTER_LABEL_LEFT_RATIO
    local percentX = rect.x + rect.w * FOOTER_PERCENT_RIGHT_RATIO
    local trackLeft = rect.x + rect.w * FOOTER_TRACK_LEFT_RATIO
    local trackRight = rect.x + rect.w * FOOTER_TRACK_RIGHT_RATIO
    ...
    painter:Text(labelX, labelY, "牛顿怒气", FOOTER_TITLE_SIZE, COLORS.dark,
        NVG_ALIGN_LEFT + NVG_ALIGN_TOP, FONT)
    painter:Text(percentX, labelY - 3,
    ...
    local travelStartX = track.x + ANGER_APPLE_SIZE * 0.5
    local travelWidth = math.max(0, track.w - ANGER_APPLE_SIZE)
    local appleCenterX = snapToPixel(travelStartX + travelWidth * progress, pixelScale)
    if progress > 0.001 then
        local fillWidth = appleCenterX - track.x
