...
def _to_lua(value: Any, depth: int = 0) -> str:
    indent = "    " * depth
    ...
    if isinstance(value, list):
        if not value:
            return "{}"
        body = ",\n".join(f"{child_indent}{_to_lua(item, depth + 1)}" for item in value)
        return "{\n" + body + f"\n{indent}}}"
    if isinstance(value, dict):
        if not value:
            return "{}"
        body = ",\n".join(
            f"{child_indent}{_lua_key(str(key))} = {_to_lua(value[key], depth + 1)}"
...
def build(spec_path: Path) -> dict[str, Any]:
    ...
    lua_path = project_root / spec["output"]["luaManifest"]
    json_path.parent.mkdir(parents=True, exist_ok=True)
    lua_path.parent.mkdir(parents=True, exist_ok=True)
    json_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    lua_path.write_text(
        "-- Generated by asset_pipeline/companion_runtime.py. Do not edit by hand.\n"
        "return " + _to_lua(manifest) + "\n",
        encoding="utf-8",
    )
    return manifest
